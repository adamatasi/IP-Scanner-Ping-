...
import sys, socket #import system and socket "Socket allow us to connect to socket/network"
...
def write_results(filename, ping):
...
def main(subnet, filename):
	write_results(filename, ping_subnet(subnet))

def parse_arguments():
	parser = argparse.ArgumentParser(usage='%(prog)s [options] <subnet>',
		description = 'ip checker',
		epilog = "python ipscanner.py 192.168.2 -f somefile.txt")
	...
	if " " in args.filename:
		parser.error("There connot be whitespaces in the filename")
